...
public class PreApprovedVisitor
{
    ...
    public void MarkAsEntered()
    {
        if (Status != VisitorStatus.Pending)
            throw new InvalidOperationException($"Cannot mark visitor as entered. Current status: {Status}");
    ...
    public void MarkAsExited()
    {
        if (Status != VisitorStatus.Inside)
            throw new InvalidOperationException($"Cannot mark visitor as exited. Current status: {Status}");
        Status = VisitorStatus.Exited;
    }

    // Admin override — allows entry from ANY status, no OTP check
